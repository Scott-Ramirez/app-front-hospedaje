...
export const SupervisorDashboardView: React.FC<SupervisorDashboardViewProps> = ({ data, porcentajeOcupacion }) => {
  const { resumen, alertas } = data;
  const { actividades } = useBitacora();
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-col flex-1">
            <div className="px-5 py-4 border-b border-outline-variant/60 flex items-center justify-between bg-surface-container-low">
              ...
              <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${
                estanciasVencidas.length > 0 ? 'bg-red-500/10 text-red-500 border border-red-500/20' : 'bg-emerald-500/10 text-emerald-600 border border-emerald-500/20'
              }`}>
                {estanciasVencidas.length > 0 ? `${estanciasVencidas.length} con saldo pendiente` : '\u2713 Todo al día'}
              </span>
            </div>
      ...
      <div className="bg-gradient-to-r from-primary/10 via-surface-lowest to-surface-lowest border border-outline-variant rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="space-y-1 text-center md:text-left">
          <h4 className="text-sm font-black text-primary uppercase tracking-wider">Consola de Control del Supervisor</h4>
          <p className="text-xs text-on-surface-variant leading-relaxed">Monitoreo de actividades de recepción, control de habitaciones y auditoría de egresos.</p>
        </div>
